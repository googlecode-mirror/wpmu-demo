		...
		if ( is_array($woo_options) ) {
			...
			if ( isset( $woo_options['woo_typography'] ) && $woo_options['woo_typography'] == 'true' ) {
				...
				if ( isset( $woo_options['woo_font_page_title'] ) && $woo_options['woo_font_page_title'] )
					$output .= '#top-section header h1 { '.woo_generate_font_css($woo_options[ 'woo_font_page_title' ]).' }' . "\n";
			
				if ( isset( $woo_options['woo_font_post_title'] ) && $woo_options['woo_font_post_title'] )
					$output .= '.post header h1, .post header h1 a { '.woo_generate_font_css($woo_options[ 'woo_font_post_title' ]).' }' . "\n";	
			
				if ( isset( $woo_options['woo_font_post_meta'] ) && $woo_options['woo_font_post_meta'] )
					$output .= '.post .post-meta, .type-page .post-meta, .type-feature .post-meta, .type-testimonial .post-meta, .search-results .page .post-meta, .search-results .type-product .post-meta, .search-results .type-feature .post-meta, .search-results .type-testimonial .post-meta { '.woo_generate_font_css($woo_options[ 'woo_font_post_meta' ]).' }' . "\n";	
...
add_action( 'woo_head', 'appply_custom_fonts' );
if (!function_exists( 'appply_custom_fonts') ) {
	function appply_custom_fonts() {
		echo "<link href='http://fonts.googleapis.com/css?family=Open+Sans:400,300' rel='stylesheet' type='text/css'>
";
	}
}
...
function woo_customise_search_form ( $html ) {
  ...
  $html = str_replace( ' value="Search"', ' value="' . esc_attr( __( 'Go', 'woothemes' ) ) . '"', $html );
