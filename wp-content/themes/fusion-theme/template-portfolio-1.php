<?php
/*
Template Name: 1 Column Portfolio
*/
?>
<?php get_header(); ?>
<?php $filter = get_post_meta($post->ID, 'filter_value', true);?>
        <div id="main_wrap">
            ...
            <div id="main">
                <div class="container">
                    <div class="sixteen columns _portfolio-1 function1">
                        <?php
                        ...
                        <div id="portfolio" class="portfolio-1">
                            ...
                            <li class="sixteen columns item all <?php if($terms) : foreach ($terms as $term) { echo ''.$term->name.' '; } endif; ?>">
                                <?php if ( $lightbox) { ?>
                                <div class="img twelve columns alpha">
                                    <a rel="prettyPhoto[pp_gal-<?php echo $post->ID ?>]" href="<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' ); echo $image[0];?>">
                                        <img class="portfolio-overlay-item-images" src="<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), '700x465' ); echo $image[0];?>" width="460" height="306"  alt="<?php _e('view more', GETTEXT_DOMAIN) ?>" />
                                    </a>
                                </div>
                                ...
                                <?php } elseif ( $linkURL ) { ?>
                                    <?php if (  (function_exists('has_post_thumbnail')) && (has_post_thumbnail())  ) { ?>
                                        <div class="img twelve columns alpha"><img class="portfolio-overlay-item-link" src="<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), '700x465' ); echo $image[0];?>" width="700" height="465"  alt="<?php _e('view more', GETTEXT_DOMAIN) ?>" rel="<?php echo $linkURL; ?>"/></div>
                                    <?php }?>
                                <?php } elseif ( $videoURL ) { ?>
                                    <div class="vimeoOrYoutube twelve columns alpha"><iframe height="306" width="460" src="<?php echo $videoURL ?>" frameborder="0" allowfullscreen></iframe></div>
                                <?php } elseif ( (function_exists('has_post_thumbnail')) && (has_post_thumbnail()) ) { ?>
                                    <div class="img twelve columns alpha"><img class="portfolio-overlay-item" src="<?php $image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), '700x465' ); echo $image[0];?>" width="700" height="465"  alt="<?php _e('view more', GETTEXT_DOMAIN) ?>" rel="<?php the_permalink(); ?>"/></div>
                                <?php }?>
                                <div class="info four columns omega">
                                    <?php if ( $videoURL ) { ?>
                                    ...
                                    <?php the_excerpt(); ?>
                                    <?php if ($linkURL) { ?>
                                    <a class="small_button" href="<?php echo $linkURL; ?>"><?php _e('view more', GETTEXT_DOMAIN) ?></a>
                                    <?php }elseif(!$lightbox){?>
                                    <a class="small_button" href="<?php the_permalink(); ?>"><?php _e('view more', GETTEXT_DOMAIN) ?></a>
                                    <?php }?>
                                </div>
                            </li>
                            <?php endwhile; endif; ?>
