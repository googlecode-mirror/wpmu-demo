	...
	init : function() {
		...
		jQuery('#layerslider-tabs span.ui-icon-close').live('click', function() {
			if(confirm('Are you sure to remove this slider?')) {
				...
				// Iterate over the tabs and re-index their names
				jQuery('.ui-tabs-nav li').each(function() {
					var index = jQuery(this).index() + 1;
					jQuery(this).find('a').html('<i class="ls-hidden">LayerSlider </i> #'+index+'');
				});
			}
		});
...
*/
        ...
        jQuery('input[name="yourlogostyle"]').live('keyup', function() {
        	...
        	slider.find('.layerslider-yourlogo-img').each(function() {
	...
	submit : function(ele) {
		...
		jQuery('#layerslider-tabs > div').each(function(slide) {
			...
			jQuery(this).find('.form-table input, .form-table textarea').each(function() {
...
jQuery(document).ready(function() {
	...
	window.send_to_editor = function(html) {
		...
		if(jQuery(uploadInput).is('input[name="background"]')) {
			...
			jQuery(uploadInput).closest('table').next().find('.draggable').css('background-image', 'url('+jQuery(uploadInput).val()+')');
